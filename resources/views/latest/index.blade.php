                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td class="px-4 sm:px-6 py-4 text-gray-900">
                                {{-- {{ \Carbon\Carbon::parse($item->date_time_late)->format('d M Y H:i') }} --}}
                                {{ ucwords(date('d M Y ', strtotime($item['date_time_late']))) }}
                            </td>
                            <td class="px-4 sm:px-6 py-4 text-gray-900">
                                {{ ucwords($item['information']) }}
                            </td>
                            <td class="px-4 sm:px-6 py-4">
                                <a href="{{ route('latest.edit', $item['id']) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                                <button href="#" data-modal-target="popup-modal" data-modal-toggle="popup-modal" class="text-red-600 bg-white hover:bg-slate-100 font-medium rounded-lg text-sm px-4 sm:px-2.5 py-2.5 text-center dark:bg-white dark:hover:bg-slate-100" type="button">
                                    Hapus
                                </button>

                                <div id="popup-modal" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full sm:inset-0 h-[calc(100%-1rem)] max-h-full">
                                    <div class="relative p-4 w-full max-w-md max-h-full">
                                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                            <form method="POST" action="{{ route('latest.delete', $item['id']) }}">
                                                @csrf
                                                @method('DELETE')

                                                <button type="button" class="absolute top-3 sm:end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="popup-modal">
                                                ...
                                                <div class="p-4 sm:p-5 text-center">
                                                    ...
                                                    <button type="submit" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 sm:px-2.5 py-2.5 text-center me-2">
                                                        Ya
                                                    </button>
                                                    <button type="button" data-modal-hide="popup-modal" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 sm:px-2.5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">Batal</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
